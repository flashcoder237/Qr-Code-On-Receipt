// src/components/organisms/centre-attestation-generator/CentreAttestationExportOptions.tsx
// Options d'export pour les attestations de centre
...
import { validateCentreAttestationList } from './attestation-validator';

interface CentreAttestationExportOptionsProps {
  students: CentreAttestationStudentRecord[];
  exportFormat: string;
  useCompression: boolean;
  onExportFormatChange: (format: string) => void;
  onUseCompressionChange: (enabled: boolean) => void;
  onGenerateAttestations: (validStudents: CentreAttestationStudentRecord[]) => void;
  isLoading: boolean;
}

export const CentreAttestationExportOptions: React.FC<CentreAttestationExportOptionsProps> = ({
  students,
  exportFormat,
  useCompression,
  onExportFormatChange,
  onUseCompressionChange,
  onGenerateAttestations,
  isLoading
}) => {
  const [showValidationDetails, setShowValidationDetails] = useState(false);

  // Valider les attestations
  const validation = validateCentreAttestationList(students);
  const { valid, invalid, stats } = validation;

  const handleGenerate = () => {
    // Ne génère que les attestations valides
    const validStudents = valid.map(v => v.student);
    onGenerateAttestations(validStudents);
  };
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-blue-700">
              {stats.canGenerate} attestation(s) seront générée(s)
            </span>
            <Button
              variant="link"
              size="sm"
          ...
          {showValidationDetails && invalid.length > 0 && (
            <div className="mt-3 border-t pt-3">
              <h5 className="font-medium text-sm text-gray-700 mb-2">Attestations exclues :</h5>
        ...
        {stats.canGenerate === 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Aucune attestation ne peut être générée. Tous les étudiants ont des données manquantes.
            </AlertDescription>
          </Alert>
        )}
        ...
        {stats.canGenerate > 0 && stats.canGenerate < stats.total && (
          <Alert className="border-yellow-300 bg-yellow-50">
            ...
            <AlertDescription className="text-yellow-800">
              <strong>{stats.total - stats.canGenerate} attestation(s)</strong> seront exclues de la génération
              (données incomplètes).
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="attestation-compression">Compression avancée</Label>
            <Switch
              id="attestation-compression"
        ...
        <div className="bg-gray-50 border rounded-lg p-3 text-sm">
          ...
          <ul className="list-disc list-inside space-y-1 text-gray-700">
            <li>Tous les champs obligatoires doivent être remplis (11 champs)</li>
            <li>Les attestations non conformes seront listées dans le rapport</li>
          </ul>
        </div>
